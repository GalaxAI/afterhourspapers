...
class GPT(nn.Module):
    def __init__(self, model_args: Config, custom_ffn=None):
        ...
        if not hasattr(torch.nn.functional, "scaled_dot_product_attention"):
            ...
            mask = torch.tril(torch.ones(model_args.seq_length, model_args.seq_length, device=device)).view(1, 1, model_args.seq_length, model_args.seq_length)
...
class SelfAttention(nn.Module):
    def __init__(self, model_args: Config):
        ...
        self.results_dropout = nn.Dropout(model_args.dropout)
...
class FeedForwardNetwork(nn.Module):
    def __init__(self, model_args: Config):
        ...
        self.ffn = nn.Sequential(
            nn.Linear(model_args.n_embd, model_args.n_embd * 4), nn.GELU(), nn.Linear(model_args.n_embd * 4, model_args.n_embd), nn.Dropout(model_args.dropout)
        )
...
class Block(nn.Module):
    def __init__(self, model_args: Config, custom_ffn=None):
        ...
        self.ffn = custom_ffn(model_args) if custom_ffn else FeedForwardNetwork(model_args)
...
for step in tqdm(range(max_iters), desc="Training Progress"):
    if step % eval_interval == 0 or step == max_iters - 1:
        losses = estimate_loss()
        print(f"\nstep {step}: train loss {losses['train']:.4f}, val loss {losses['val']:.4f}")

        wandb.log({"step": step, "train_loss": losses["train"], "val_loss": losses["val"]})
